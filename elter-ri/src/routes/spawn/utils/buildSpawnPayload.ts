...
/** Apply `delhome` when the delete home directory toggle is enabled. */
export function applyDelhome(
  payload: SpawnFormPayload,
  formData: SpawnFormData,
): SpawnFormPayload {
  if (!formData.delhome) return payload;
  return { ...payload, delhome: "yes" };
}
