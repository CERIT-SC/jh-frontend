...
export function ImageSelectionSectionTabs({
  ...
  useEffect(() => {
    ...
    // No saved/default image: auto-select first category and first image in that category
    const categoryKeys = Object.keys(images);
    if (categoryKeys.length > 0) {
      const firstCategory = categoryKeys[0];
      const firstImageValue = images[firstCategory][0]?.value;

      if (firstImageValue) {
        const formImagesMap = formImagesName as Record<string, string>;
        const formImageKey = formImagesMap[firstCategory] || firstCategory;

        setSelectedCategory(firstCategory);
        setSelectedImage(firstImageValue);
        onImageChange?.({
          images: firstCategory,
          [formImageKey]: `cerit.io/hubs/${firstImageValue}`,
        });
      }
    }
  }, []);
